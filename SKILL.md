...
- **Escreve:** `{cliente}/Posts/CALENDÁRIO OFICIAL – [MÊS].md` — inclui a seção `## Google Meu Negócio`, escrita automaticamente por `/criar-copy-gmn` durante o lote, não por esta skill diretamente. Também escreve a coluna `Composição proposta` da tabela, quando o fluxo de planejamento autorizar (ver Passo 2, item 4) — esta skill **nunca escreve `Composição usada`, `Layout` de resultado real ou `Arquivo` de resultado real, e nunca inventa uso real** — cria e planeja linhas, nunca registra resultado produtivo. O registro pós-exportação é responsabilidade de quem gera a arte, conforme o `Formato` da linha: `/criar-post-instagram` registra `Composição usada` para post único (ver `criar-post-instagram/SKILL.md` → "Registrar Composição usada"); `/criar-carrossel-instagram` registra `Layout`, `Composição usada` e `Arquivo` juntos, numa única substituição da linha, para carrossel (ver `criar-carrossel-instagram/SKILL.md` → "Registrar resultado final no calendário") — em ambos os casos, só depois da exportação final real daquele post/carrossel. Nenhuma das três skills move automaticamente uma linha entre `Planejados` e `Publicados`, nenhuma usa aprovação visual, protótipo ou intenção como prova de exportação/publicação, e nenhuma embute ID estruturado (`OFICIAL-*`/`CMP-*`/`OFICIALC-*`/`CMPC-*`) dentro de `Layout`
...
- Se o calendário do cliente usar seções de status (ex: `## Planejados` / `## Publicados`), mover uma linha entre elas nunca altera o schema da tabela: preservar `Formato`, `Tipo`, `Layout`, `Composição proposta`, `Composição usada` e `Arquivo` como células próprias, nunca embutir ID estruturado (`OFICIAL-*`/`CMP-*`/`OFICIALC-*`/`CMPC-*`) dentro do texto livre de `Layout`. Calendário legado sem esse schema continua compatível; migração retroativa de linhas antigas só ocorre com autorização explícita e quando os dados permitirem separação segura — nunca inferir `Tipo` ou `Composição usada` pela aparência de uma descrição ambígua

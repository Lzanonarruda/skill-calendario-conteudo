...
- **Escreve:** `{cliente}/Posts/CALENDÁRIO OFICIAL – [MÊS].md` — inclui a seção `## Google Meu Negócio`, escrita automaticamente por `/criar-copy-gmn` durante o lote, não por esta skill diretamente. Também escreve a coluna `Composição proposta` da tabela, quando o fluxo de planejamento autorizar (ver Passo 2, item 4) — esta skill **nunca escreve `Composição usada` e nunca inventa uso real**. O registro de `Composição usada` é responsabilidade de quem gera a arte, conforme o `Formato` da linha: `/criar-post-instagram` para post único, `/criar-carrossel-instagram` para carrossel — em ambos os casos, só depois da exportação final real daquele post/carrossel (ver `criar-post-instagram/SKILL.md` e `criar-carrossel-instagram/SKILL.md` → "Registrar Composição usada"). Nenhuma das três skills move automaticamente uma linha entre `Planejados` e `Publicados`, nenhuma usa aprovação visual, protótipo ou intenção como prova de exportação/publicação, e nenhuma embute ID estruturado (`OFICIAL-*`/`CMP-*`/`OFICIALC-*`/`CMPC-*`) dentro de `Layout`
